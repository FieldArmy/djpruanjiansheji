  ...
  <div>
    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix" style="height: 30px">
        <span class="size_t1">模拟选报</span>
        <br>
        <el-button type="text" @click="dialogVisible = true"> 【注意事项】</el-button>
      </div>
      <el-dialog title="注意事项" :visible.sync="dialogVisible" width="300px">
        <h3 style="margin-top: -20px">本系统根据2021和2022两年录取数据，进行推荐志愿和预测录取风险.</h3>
        <h3>本系统推荐的院校名单，仅供参考，报考风险由考生本身承担！</h3>
        <h3>本系统的模拟志愿单改为电脑端，手机端只显示用户在电脑端填写的信息并根据信息显示能够上的大学</h3>
      </el-dialog>
      <!-- 城市级联选择器-->
      <div class="block">
        <div>
          <!--填报批次-->
          <el-button type="text" style="margin-top: 5px"><h3>填报批次</h3></el-button>
          ...
          <!-- 城市 -->
          <br>
          <el-button type="text" style="margin-top: 5px;margin-left: 20px"><h3>就读地区</h3></el-button>
          <el-cascader
              placeholder="不限"
              class="width_form"
              :options="options1"
          ...
          <!-- 本科专业-->
          <el-button type="text" style="margin-top: 5px;margin-left: 15px"><h3>专业类别</h3></el-button>
          <el-cascader
              placeholder="不限"
              class="width_form"
              :options="options2"
          ...
          <el-cascader
              ...
              collapse-tags
              :props="{ multiple: true, expandTrigger: 'hover' }"
              v-if="choose_major(button_checked) == 2"
          >
          </el-cascader>
          <!-- 专业类别-->
          <el-button type="text" style="margin-top: 5px;margin-left: 15px"><h3>大学类型</h3></el-button>
          <el-cascader
              class="width_form"
              placeholder="不限"
              :options="options4"
          ...
          <el-button type="text" style="margin-top: 5px;margin-left: 15px"><h3>个人情况</h3></el-button>
          <el-cascader
              class="width_form"
              placeholder="不限"
              :options="options5"
          ...
          <el-button type="text" style="margin-top: 5px;margin-left: 15px"><h3>是否临海</h3></el-button>
          <el-switch
              v-model="value"
              active-color="#13ce66"
          ...
          <el-button type="primary" plain v-if="choose_major(button_checked) == 1" icon="el-icon-search" @click="get1()"
                     style="margin-left: 20px">提交
          </el-button>
          <el-button type="primary" plain v-if="choose_major(button_checked) == 2" icon="el-icon-search" @click="get2()"
                     style="margin-left: 20px">提交
          </el-button>
        </div>
    ...
    <div>
      <!--显示的表格-->
      <el-card class="box-card2" id="id1">
        <el-table :data="info.slice((currentPage-1)*pageSize,currentPage*pageSize)" style="margin-top: 0" stripe>
          <el-table-column prop="probability" align="center" label="录取预测" class="table-h">
            <template slot-scope="scope">
              <span v-if="scope.row.probability=='风险'" class="a">风险</span>
              <span v-if="scope.row.probability=='冲刺'" class="a">冲刺</span>
              <span v-if="scope.row.probability=='较稳'" class="a">较稳</span>
              <span v-if="scope.row.probability=='保底'" class="a">保底</span>
            </template>
          </el-table-column>
          <el-table-column label="2021招生计划" class="Fill_size" align="center">
            <el-table-column prop="major" align="center" label="专业" width="180" class="table-h">
              <template slot-scope="scope">
                <span class="a">{{ scope.row.major }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="universities" align="center" label="学校名称">
              <template slot-scope="scope">
                <router-link :to="'int'+'?id='+scope.row.school_id" class="b">
                  <span>{{ scope.row.universities }}</span>
                </router-link>
              </template>
            </el-table-column>
            <el-table-column prop="plan" align="center" label="计划数" class="table-h">
              <template slot-scope="scope">
                <span class="a">{{ scope.row.plan }}</span>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column prop="lowest_score" align="center" label="最低分" width="180" class="table-h">
            <template slot-scope="scope">
              <span class="a">{{ scope.row.lowest_score }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="lowest_ranking" align="center" label="最低位次" class="table-h">
            <template slot-scope="scope">
              <span class="a">{{ scope.row.lowest_ranking }}</span>
            </template>
          </el-table-column>
          <el-table-column label="详细信息" align="center">
            <el-table-column prop="school_chengshi" align="center" label="地址" class="table-h">
              <template slot-scope="scope">
                <span class="a">{{ scope.row.school_chengshi }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="school_type_cc" align="center" label="属性" class="table-h">
              <template slot-scope="scope">
                <span class="a">{{ scope.row.school_type_cc }}</span>
              </template>
            </el-table-column>
          </el-table-column>
          <el-table-column prop="limit1" align="center" label="身体限制" class="table-h">
            <template slot-scope="scope">
              <span class="a">{{ scope.row.limit1 }}</span>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      list4: "*",
      list11: "-北京市,-天津市",
      list22: "哲学类,-哲学,经济学,-经济学类,经济学,-财政,经济学,-金融,经济学,-经济与贸易,法学,-法学,法学,-政治学,法学,-社会学,法学,-民族学,法学,-马克思学,法学,-公安学",
      list33: "大学特色,-211,办学性质,-公办",
      list44: "不限",
      total: 0,
      pageSize: 10,  //一页显示多少条
      ...
      options5: [
        {value: '不限', label: '不限',},
        {value: '色弱', label: '轻度色觉异常(色弱)',},
        {value: '色盲', label: '色觉异常(色盲)',},
        {value: '不能准确识别', label: '不能准确识别红、黄、绿、蓝、紫各颜色中任何一种颜色',},
      ]
    }
  },
  ...
  methods: {
    ...
    get1() {
      ...
      if (this.list4 == '') this.list4 = '不限';
      axios.get("http://101.34.248.53:8181/Map/list/zyzhcx/" + '本科' + '/' + this.username +'/' + this.list1 + '/' + this.list2 + '/' + this.list3 + '/' + this.list4 + '/' + this.value)
          .then(resp => {
            // console.log(resp);
            this.info = resp.data;
    ...
    get2() {
      ...
      if (this.list3 == '') this.list3 = '*';
      if (this.list4 == '') this.list4 = '不限';
      axios.get("http://101.34.248.53:8181/Map/list/zyzhcx/" + "专科" + '/' + this.username +'/' + this.list1 + '/' + this.list2 + '/' + this.list3 + '/' + this.list4 + '/' + this.value)
          .then(resp => {
            console.log(resp);
            this.info = resp.data;
            this.total = resp.data.length;
    ...
    //初始化显示的数据
    getSchool() {
      axios.get("http://101.34.248.53:8181/Map/list/zyzhcx/" + "本科" + "/" + this.username + '/' + this.list11 + '/' + this.list22 + '/' + this.list33 + '/' + this.list44 + '/' + this.num2)
          .then(resp => {
            this.info = resp.data;
            this.total = resp.data.length;
          })
    ...
    get_Axios(){
      let get_data;
      this.axios('http://101.34.248.53:8181/Map/Voluntary/Voluntary_Info_if/' + this.username,{
        method:'get'
      }).then((resp)=>{
        get_data = resp.data;
        this.show = get_data.State;
        this.rank = get_data.Rank;
        this.mark = get_data.Mark;
        this.sub = get_data.Subjects.split(',');
