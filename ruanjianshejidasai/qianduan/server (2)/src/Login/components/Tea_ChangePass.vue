...
<div>
  <el-card class="box-card">
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="form">
      ...
      <el-button type="primary" @click="get_pass('ruleForm')" class="btn1">确认修改</el-button>
  ...
  methods: {
    ...
    get_pass() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          //Map/teacher/re_password/{username}/{password}/{password_new}
          this.ruleForm.username = this.$cookies.get('user');
          console.log("账号/密码/新密码")
          console.log(this.ruleForm.username + '/' + this.ruleForm.password + '/' + this.ruleForm.new_password)
          axios.post("http://101.34.248.53:8181/Map/teacher/re_password/"+this.ruleForm.username + '/' + this.ruleForm.password + '/' + this.ruleForm.new_password).then(resp => {
            console.log(resp);
            if (resp.data == "ERROR_Password") {
...
.login_page {
  width: 100%;
  height: 100%;
  background: url(https://pic.imgdb.cn/item/613a140244eaada73973b22c.jpg) ;
